    ...
    public class Player : LivingCreature
    {
        ...
        public static Player CreatePlayerFromXmlString(string xmlPlayerData)
        {
            try
            {
                ...
                foreach (
                    XmlNode node in playerData.SelectNodes("/Player/InventoryItems/InventoryItem")
                )
                {
        ...
        public bool HasAllQuestCompletionItems(Quest quest)
        {
            ...
            foreach (QuestCompletionItem qci in quest.QuestCompletionItems)
            {
                ...
                if (
                    !Inventory.Any(ii =>
                        ii.Details.ID == qci.Details.ID && ii.Quantity >= qci.Quantity
                    )
                )
                {
                    return false;
            ...
            // If we got here, then the player must have all the required items, and enough of them, to complete the quest.3
            return true;
        }
        ...
        public void MoveTo(Location newLocation)
        {
            ...
            if (newLocation.QuestAvailableHere != null)
            {
                ...
                bool playerAlreadyCompletedQuest = CompletedThisQuest(
                ...
                if (playerAlreadyHasQuest)
                {
                    ...
                    {
                        // See if the player has all the items needed to complete the quest
                        bool playerHasAllItemsToCompleteQuest = HasAllQuestCompletionItems(
                            newLocation.QuestAvailableHere
                        );
                ...
                else
                {
                    ...
                    foreach (
                        QuestCompletionItem qci in newLocation
                            .QuestAvailableHere
        ...
        public string ToXmlString()
        {
            ...
            currentHitPoints.AppendChild(
                playerData.CreateTextNode(this.CurrentHitPoints.ToString())
            );
            stats.AppendChild(currentHitPoints);

            XmlNode maximumHitPoints = playerData.CreateElement("MaximumHitPoints");
            maximumHitPoints.AppendChild(
                playerData.CreateTextNode(this.MaximumHitPoints.ToString())
            );
            ...
            experiencePoints.AppendChild(
                playerData.CreateTextNode(this.ExperiencePoints.ToString())
            );
            stats.AppendChild(experiencePoints);

            XmlNode currentLocation = playerData.CreateElement("CurrentLocation");
            currentLocation.AppendChild(
                playerData.CreateTextNode(this.CurrentLocation.ID.ToString())
            );
